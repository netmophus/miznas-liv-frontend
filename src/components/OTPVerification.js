...
const OTPVerification = () => {
  ...
  useEffect(() => {
    ...
    if (pendingPhone) {
      setTelephone(pendingPhone);
    } else {
      // Si pas de numéro, rediriger vers login
      navigate('/login');
    }
  }, [navigate]);
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await authService.sendOTP(telephone);
      if (response.success) {
        setCountdown(60);
        setOtp(['', '', '', '', '', '']);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authService.verifyOTP(telephone, otpCode);
